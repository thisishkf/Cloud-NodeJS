...
app.post('/rate',function(req,res) {
	MongoClient.connect(mongourl,function(err,db) {
		assert.equal(err,null);
		checkRate(db,req.session.username,function(result) {
			if(result !=null){
				res.sendFile(__dirname + '/public/error.html');
			}
			else{	
...
function checkRate(db,owner,callback) {
		console.log(owner);
		db.collection('res').findOne({ "rate": { $elemMatch: { "owner": owner } } },function(err,result) {
			assert.equal(err,null);
			callback(result);
			console.log(result);
		}//end function(err,result) {
